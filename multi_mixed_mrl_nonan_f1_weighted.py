""" definitions for experiment 3 module
    ...
    f1_weighted metric
    Used in experiment 3 of the master's dissertation.
"""
...
class Experiment(BaseExperiment):
    ...
    def raw_transform(self, event, transient_only=True, no_nans=True):
        ...
        # if MAEDataset.TRANSIENT_CLASS[event_type]: 
    ...
    def metric_rf(self):
        return get_scorer("f1_weighted")   # alterado em 11/12/23 para balanced_accuracy

    def metric_lgbm(self):
        def acc(preds, train_data):
            preds_ = np.argmax(np.reshape(preds, (self.num_classes, -1)), axis=0)
            return "f1_weighted", f1_score(train_data.get_label(), preds_, average="weighted"), True  
